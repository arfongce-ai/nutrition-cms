...
1. 앱 실행 즉시 전체 화면 카메라
2. 먹을 음식 또는 식단을 촬영
3. 촬영된 사진 아래에서 음식명과 먹은 양(g)을 먼저 입력
4. 포장식품이면 식품 영양표의 열량, 탄수화물, 단백질, 지방, 당류, 나트륨도 함께 입력
5. 음식 분석값과 식품 영양표 입력값이 함께 합산되어 KDRI/질환/운동 목적 기준 A4 리포트 카드 즉시 갱신
6. 저장 버튼으로 리포트 저장

현재 단계에서는 실제 AI 음식 자동 인식 전 단계로, 촬영 사진을 남기고 음식명/양을 입력하면 로컬 음식 데이터 기준으로 영양을 추정합니다. 식품 영양표는 보조 입력이며, 포장식품 분석 정확도를 높일 때 함께 사용합니다.
