...
5. 촬영 즉시 `촬영 음식 250g` 자동 추정값으로 음식 분석을 먼저 계산
6. 정확도를 높이고 싶을 때만 음식명과 먹은 양(g)을 수정
7. 포장식품이면 식품 영양표의 열량, 탄수화물, 단백질, 지방, 당류, 나트륨도 함께 입력
8. 음식 분석값과 식품 영양표 입력값이 함께 합산되어 KDRI/질환/운동 목적 기준 A4 리포트 카드 즉시 갱신
9. 저장 버튼으로 리포트 저장

현재 단계에서는 촬영 직후 자동 추정값으로 A4 리포트가 바로 생성됩니다. 실제 AI 음식 자동 인식 전 단계이므로 정확도를 높이려면 음식명/양을 선택적으로 수정할 수 있습니다. 식품 영양표는 보조 입력이며, 포장식품 분석 정확도를 높일 때 함께 사용합니다. 영양표 자동 인식은 브라우저의 `TextDetector` 지원 기기에서 동작하며, 미지원 기기에서는 촬영 후 열량과 주요 영양소 칸에 직접 입력할 수 있습니다.
